...
static const char * const es8388_supply_names[] = {
	"DVDD",
	"PVDD",
	"AVDD",
	"HPVDD",
...
struct es8388_priv {
    ...
    struct regulator_bulk_data supplies[ARRAY_SIZE(es8388_supply_names)];
...
static int es8388_set_gpio(int gpio, bool level)
{
    ...
    if ((gpio & ES8388_CODEC_SET_SPK) && es8388
        && es8388->spk_ctl_gpio != NULL) {
        gpiod_set_value(es8388->spk_ctl_gpio, level);
    }
...
static irqreturn_t hp_det_irq_handler(int irq, void *dev_id)
{
    ...
    if (gpiod_get_value(es8388->hp_det_gpio))
...
static const struct snd_kcontrol_new es8388_left_mixer_controls[] = {
    ...
    // SOC_DAPM_SINGLE("Left Playback Switch", SND_SOC_NOPM, 7, 1, 0),
    // SOC_DAPM_SINGLE("Left Bypass Switch", SND_SOC_NOPM, 6, 1, 0),
};
...
static const struct snd_kcontrol_new es8388_right_mixer_controls[] = {
    ...
    // SOC_DAPM_SINGLE("Right Playback Switch", SND_SOC_NOPM, 7, 1, 0),
    // SOC_DAPM_SINGLE("Right Bypass Switch", SND_SOC_NOPM, 6, 1, 0),
};
		      ...
		      struct snd_kcontrol *kcontrol, int event)
{
	...
	case SND_SOC_DAPM_PRE_PMU:
		return clk_prepare_enable(es8388->mclk);
...
static const struct snd_soc_dapm_widget es8388_dapm_mclk_widgets[] = {
                ...
                struct snd_soc_dai *dai)
{
    ...
    if ((es8388->sysclk/params_rate(params) == 256) |
        ...
        /* fast charge only */
        snd_soc_write(codec, 0x37, 0xc0);
        snd_soc_write(codec, 0x39, 0xc0);
    }
...
static int es8388_of_xlate_dai_id(struct snd_soc_component *component,
...
static int es8388_probe(struct snd_soc_codec *codec)
{
	...
	snd_soc_dapm_new_controls(dapm, es8388_dapm_mclk_widgets, 1);
...
static struct snd_soc_codec_driver soc_codec_dev_es8388 = {
	...
	.of_xlate_dai_id	= es8388_of_xlate_dai_id,
	.idle_bias_on		= 1,
	.use_pmdown_time	= 1,
	.endianness		= 1,
};
...
static int es8388_i2c_probe(struct i2c_client *i2c)
{
    ...
    es8388->spk_ctl_gpio = devm_gpiod_get_optional(&i2c->dev,"spk-con",GPIOD_OUT_HIGH);
	if (IS_ERR(es8388->spk_ctl_gpio)) {
        dev_err(&i2c->dev, "Failed to get spk-con-gpio descriptor\n");
    	return PTR_ERR(es8388->spk_ctl_gpio);
    }
    
    es8388->hp_det_gpio = devm_gpiod_get_optional(&i2c->dev, "hp-det", GPIOD_IN);
    if (IS_ERR(es8388->hp_det_gpio)) {
        dev_err(&i2c->dev, "Failed to get hp-det-gpio descriptor\n");
		return PTR_ERR(es8388->hp_det_gpio);
    }

    hp_irq = gpiod_to_irq(es8388->hp_det_gpio);

    if (hp_irq > 0) {
		ret = devm_request_threaded_irq(&i2c->dev, hp_irq, NULL,
						hp_det_irq_handler, IRQ_TYPE_EDGE_BOTH  | IRQF_ONESHOT ,"ES8388", es8388);
		if (ret < 0) {
	...
	for (int i = 0; i < ARRAY_SIZE(es8388->supplies); i++)
		es8388->supplies[i].supply = es8388_supply_names[i];

	ret = devm_regulator_bulk_get(&i2c->dev, ARRAY_SIZE(es8388->supplies),
				      es8388->supplies);
	if (ret != 0) {
	...
	ret = regulator_bulk_enable(ARRAY_SIZE(es8388->supplies),
				    es8388->supplies);
	if (ret != 0) {
